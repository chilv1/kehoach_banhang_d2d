        ...
        <tbody>
          {tasks.map((t) => (
            <tr key={t.id} className={[t.is_critical ? "critical" : "", t.is_milestone ? "milestone" : ""].join(" ")}>
              ...
              <td>{fmt(t.finish_date)}</td>
              <td style={{ color: t.is_critical ? "#dc2626" : "#6b7280" }}>
                {t.total_slack_hours.toFixed(1)}h
              </td>
      ...
      {predDialog && (
        <div style={{
          ...
          <div className="card" style={{ padding: 20, minWidth: 360 }}>
            ...
            <input
              autoFocus placeholder="Predecessor task ID"
              value={predDialog.value}
