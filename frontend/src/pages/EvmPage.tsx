        ...
        {d && (
          <div style={{ padding: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              <Box label="BAC (Budget)" value={fmtMoney(d.bac)} />
              <Box label="BCWS (Planned Value)" value={fmtMoney(d.bcws)} />
              <Box label="BCWP (Earned Value)" value={fmtMoney(d.bcwp)} />
              <Box label="ACWP (Actual Cost)" value={fmtMoney(d.acwp)} />
              <Box label="CV (Cost Variance)" value={fmtMoney(d.cv)} color={d.cv >= 0 ? "#16a34a" : "#dc2626"} />
              <Box label="SV (Schedule Var)" value={fmtMoney(d.sv)} color={d.sv >= 0 ? "#16a34a" : "#dc2626"} />
              <Box label="CPI" value={fmtIdx(d.cpi)} color={indexColor(d.cpi)} />
              <Box label="SPI" value={fmtIdx(d.spi)} color={indexColor(d.spi)} />
              <Box label="EAC (Estimate)" value={fmtMoney(d.eac)} />
              <Box label="ETC (To Complete)" value={fmtMoney(d.etc)} />
              <Box label="VAC (Variance at Completion)" value={fmtMoney(d.vac)} color={d.vac && d.vac >= 0 ? "#16a34a" : "#dc2626"} />
            ...
            <table>
              ...
              <tbody>
                {d.tasks.map((t: any) => (
                  <tr key={t.task_id}>
                    <td>{t.name}</td>
                    <td>{t.percent_complete.toFixed(0)}%</td>
                    <td>{fmtMoney(t.planned_cost)}</td>
                    <td>{fmtMoney(t.bcws)}</td>
                    <td>{fmtMoney(t.bcwp)}</td>
                    <td>{fmtMoney(t.acwp)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
