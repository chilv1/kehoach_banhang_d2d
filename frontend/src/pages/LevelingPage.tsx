...
export default function LevelingPage() {
  ...
  const overQ = useQuery({ queryKey: ["over", projectId], queryFn: () => getOverallocations(projectId) });
  const loadQ = useQuery({ queryKey: ["resload", projectId], queryFn: () => getResourceLoad(projectId) });
      ...
      <div className="card">
        <div className="toolbar">
          ...
          <span style={{ marginLeft: "auto" }}>
            <button className="primary" onClick={() => levelMut.mutate()} disabled={levelMut.isPending}>
        ...
        {levelMut.data && (
          <div style={{ padding: 12, background: "#f0fdf4", borderBottom: "1px solid #e5e7eb" }}>
            ✅ Iterations: {levelMut.data.iterations}, converged: {String(levelMut.data.converged)}, delays: {levelMut.data.delays.length}
          </div>
        )}
        <div style={{ padding: 16 }}>
          ...
          {(overQ.data?.length ?? 0) === 0 ? (
            ...
            <table>
              <thead><tr><th>Resource</th><th>Ngày</th><th>Used</th><th>Max</th><th>Over</th></tr></thead>
              <tbody>{overQ.data.map((o: any, i: number) => (
                <tr key={i} className="critical">
                  <td>{o.resource_name}</td><td>{o.date}</td><td>{o.used_units}</td><td>{o.max_units}</td>
                  <td style={{ color: "#dc2626", fontWeight: 600 }}>+{o.over}</td>
                </tr>))}
              </tbody>
            </table>
          )}
          <h3 style={{ marginTop: 24 }}>📊 Resource daily load</h3>
          <table>
            <thead><tr><th>Resource ID</th><th>Số ngày làm</th><th>Peak</th></tr></thead>
            <tbody>
              {Object.entries(loadQ.data ?? {}).map(([rid, days]: any) => {
                const peak = Math.max(0, ...Object.values(days as Record<string, number>));
                return (<tr key={rid}><td>{rid}</td><td>{Object.keys(days).length}</td><td>{peak.toFixed(2)}</td></tr>);
              })}
            </tbody>
          </table>
        </div>
