...
import {
  createBaseline, getProject, getVariance, listBaselines,
} from "@/api/client";
    ...
    <div className="page">
      ...
      <div className="card">
        ...
        <div style={{ padding: 16 }}>
          <h3>📚 Baselines lưu trữ</h3>
          {(baselinesQ.data?.length ?? 0) === 0 ? (
            <div style={{ color: "#6b7280" }}>Chưa có baseline. Bấm "Save baseline" để snapshot kế hoạch hiện tại.</div>
          ) : (
            <table>
              <thead><tr><th>#</th><th>Tên</th><th>Snapshot date</th></tr></thead>
              <tbody>
                {baselinesQ.data.map((b: any) => (
                  <tr key={b.id}>
                    <td>{b.number}</td>
                    <td>{b.name}</td>
                    <td>{b.snapshot_date?.slice(0, 19).replace("T", " ")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <table>
            <thead>
              <tr>
                <th>Task</th><th>Baseline start</th><th>Current start</th>
                <th>Baseline finish</th><th>Current finish</th>
                <th>Finish var (h)</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {(varianceQ.data?.tasks ?? []).map((t: any) => (
                <tr key={t.task_id} className={t.is_slipping ? "critical" : ""}>
                  ...
                  <td>
                    {t.is_slipping ? <span className="badge b-crit">SLIPPING</span> : <span className="badge b-ok">ON TIME</span>}
                  </td>
                </tr>
              ))}
            </tbody>
