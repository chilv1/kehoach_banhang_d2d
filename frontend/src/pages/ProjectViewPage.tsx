...
import {
  ...
  importXml, listDependencies, listTasks,
...
export default function ProjectViewPage() {
  ...
  const depsQ = useQuery({ queryKey: ["deps", projectId], queryFn: () => listDependencies(projectId) });
  ...
  const addTaskMut = useMutation({
    mutationFn: () => createTask({ project_id: projectId, name: "New task", duration_hours: 8,
  ...
  return (
    <div className="page">
      <div style={{ marginBottom: 8 }}><Link to="/">← Projects</Link></div>
      <div className="card">
        <div className="toolbar">
          ...
          <span style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <a href={exportXmlUrl(projectId)} download><button>⬇ XML</button></a>
        ...
        {tab === "gantt" && (
          <div className="gantt-container">
            ...
            {ganttTasks.length > 0 ? (
              <Gantt tasks={ganttTasks} viewMode={view} listCellWidth="" columnWidth={60}
                onDateChange={(t) => {
                  ...
                  updateTaskMut.mutate({
                    id,
                    data: { duration_hours: Math.max(1, Math.round(days * 8)),
                            constraint_type: "MSO",
                ...
                Chưa có task có ngày. Bấm <b>+ Task</b> rồi <b>Schedule</b>.</div>)}
          </div>
        )}
        {tab === "grid" && (
          <TaskGrid tasks={tasksQ.data ?? []} deps={depsQ.data ?? []}
