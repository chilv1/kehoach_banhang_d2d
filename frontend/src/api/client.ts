...
export const moveTask = (id: number, sort_order: number) =>
  http.post(`/tasks/${id}/move`, { sort_order }).then(r => r.data);

// Phase 3: Costs
export const recomputeCosts = (pid: number) =>
  http.post(`/projects/${pid}/costs/recompute`).then(r => r.data);
...
// Phase 5: Leveling
export const getOverallocations = (pid: number) =>
  http.get(`/projects/${pid}/overallocations`).then(r => r.data);
export const getResourceLoad = (pid: number) =>
  http.get(`/projects/${pid}/resource-load`).then(r => r.data);
export const levelResources = (pid: number) =>
  http.post(`/projects/${pid}/level`).then(r => r.data);
