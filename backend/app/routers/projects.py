...
def run_schedule(pid: int, db: Session = Depends(get_db)):
    try: res = schedule_project(db, pid)
    except SchedulingError as e: raise HTTPException(400, str(e))
    return ScheduleResult(**res)


@router.post("/tasks", response_model=TaskOut, status_code=201)
def create_task(payload: TaskCreate, db: Session = Depends(get_db)):
    obj = Task(**payload.model_dump()); db.add(obj); db.commit(); db.refresh(obj)
    return obj


@router.get("/projects/{pid}/tasks", response_model=list[TaskOut])
def list_tasks(pid: int, db: Session = Depends(get_db)):
    return db.query(Task).filter(Task.project_id == pid).order_by(Task.sort_order, Task.id).all()
...
def create_dependency(payload: DependencyCreate, db: Session = Depends(get_db)):
    pred = db.get(Task, payload.predecessor_id); succ = db.get(Task, payload.successor_id)
    if not pred or not succ: raise HTTPException(404, "Task not found")
    if pred.project_id != succ.project_id: raise HTTPException(400, "Same project required")
    if pred.id == succ.id: raise HTTPException(400, "Self-loop")
    obj = TaskDependency(**payload.model_dump()); db.add(obj); db.commit(); db.refresh(obj)
    return obj
...
def list_dependencies(pid: int, db: Session = Depends(get_db)):
    ...
    return db.query(TaskDependency).filter(TaskDependency.predecessor_id.in_(task_ids)).all()


@router.patch("/dependencies/{did}", response_model=DependencyOut)
def update_dependency(did: int, payload: DependencyUpdate, db: Session = Depends(get_db)):
...
def create_resource(payload: ResourceCreate, db: Session = Depends(get_db)):
    obj = Resource(**payload.model_dump()); db.add(obj); db.commit(); db.refresh(obj)
    return obj
...
def update_resource(rid: int, payload: ResourceUpdate, db: Session = Depends(get_db)):
...
def create_assignment(payload: AssignmentCreate, db: Session = Depends(get_db)):
    obj = Assignment(**payload.model_dump()); db.add(obj); db.commit(); db.refresh(obj)
    return obj
...
def update_assignment(aid: int, payload: AssignmentUpdate, db: Session = Depends(get_db)):
...
def create_baseline(payload: BaselineCreate, db: Session = Depends(get_db)):
    existing = db.query(Baseline).filter(
        Baseline.project_id == payload.project_id,
        Baseline.number == payload.number).first()
    if existing:
        db.delete(existing); db.commit()
    obj = Baseline(project_id=payload.project_id, number=payload.number,
                   name=payload.name or f"Baseline {payload.number}")
    db.add(obj); db.flush()
    for t in db.query(Task).filter(Task.project_id == payload.project_id).all():
        db.add(BaselineTask(baseline_id=obj.id, task_id=t.id,
                            baseline_start=t.start_date, baseline_finish=t.finish_date,
