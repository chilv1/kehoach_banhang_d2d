...
from app.services.leveling import compute_daily_load, find_overallocations, level_resources
...
def task_indent(tid: int, db: Session = Depends(get_db)):
    ...
    return {"id": t.id, "outline_level": t.outline_level, "parent_id": t.parent_id, "wbs": t.wbs}


@router.post("/tasks/{tid}/outdent")
def task_outdent(tid: int, db: Session = Depends(get_db)):
    try: t = outdent(db, tid)
    except ValueError as e: raise HTTPException(400, str(e))
    return {"id": t.id, "outline_level": t.outline_level, "parent_id": t.parent_id, "wbs": t.wbs}
...
def task_move(tid: int, payload: MovePayload, db: Session = Depends(get_db)):
    try: t = move(db, tid, payload.sort_order)
    except ValueError as e: raise HTTPException(400, str(e))
...
def get_variance(pid: int, baseline_number: int = 0, db: Session = Depends(get_db)):
    return {"baseline_number": baseline_number, "tasks": project_variance(db, pid, baseline_number)}
...
def get_resource_load(pid: int, db: Session = Depends(get_db)):
    ...
    return {str(rid): {d.isoformat(): round(u, 2) for d, u in by_day.items()}
...
def get_evm(pid: int, status_date: datetime | None = None, db: Session = Depends(get_db)):
    try: return compute_evm(db, pid, status_date)
    except ValueError as e: raise HTTPException(404, str(e))


@router.get("/projects/{pid}/export-xml")
def get_export_xml(pid: int, db: Session = Depends(get_db)):
    try: xml = export_project_xml(db, pid)
    except ValueError as e: raise HTTPException(404, str(e))
    return Response(content=xml, media_type="application/xml",
                    headers={"Content-Disposition": f'attachment; filename="project_{pid}.xml"'})


@router.post("/projects/import-xml")
async def post_import_xml(file: UploadFile = File(...), db: Session = Depends(get_db)):
    data = await file.read()
    try: pid = import_project_xml(db, data)
    except Exception as e: raise HTTPException(400, f"Lỗi parse XML: {e}")
