...
Công thức chuẩn PMI:
  BCWS (PV)  — Budgeted Cost of Work Scheduled (planned to status_date)
  BCWP (EV)  — Budgeted Cost of Work Performed (% complete × planned cost)
  ACWP (AC)  — Actual Cost of Work Performed
  CV   = BCWP - ACWP
  SV   = BCWP - BCWS
  CPI  = BCWP / ACWP
  SPI  = BCWP / BCWS
  BAC  = Budget At Completion (tổng planned cost)
  EAC  = BAC / CPI (estimate at completion)
  ETC  = EAC - ACWP (estimate to complete)
  VAC  = BAC - EAC
  TCPI = (BAC - BCWP) / (BAC - ACWP)  — to-complete CPI
"""
...
def _task_planned_cost(t: Task, assigns: list[Assignment],
                        res_map: dict[int, Resource]) -> float:
    """Planned cost = fixed + sum(work_hours × rate + cost_per_use)."""
...
def compute_evm(db: Session, project_id: int,
                 status_date: datetime | None = None) -> dict:
    ...
    for t in tasks:
        ...
        # Phần work scheduled tới status_date
        if sd >= t.finish_date:
            scheduled_frac = 1.0
        elif sd <= t.start_date:
            scheduled_frac = 0.0
        else:
