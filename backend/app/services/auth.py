"""Phase 8 — Auth service (stdlib-only JWT + pbkdf2)."""
...
def _sign(msg): return hmac.new(JWT_SECRET.encode(), msg, hashlib.sha256).digest()
...
def get_current_user_optional(token: str | None = Depends(oauth2_scheme),
    ...
    try:
        payload = jwt_decode(token); uid = int(payload.get("sub", "0"))
