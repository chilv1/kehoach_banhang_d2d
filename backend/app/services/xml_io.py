"""Phase 7 — Project XML import/export (simplified schema).

Schema kế thừa ý tưởng MS Project XML nhưng đơn giản hoá:
...
<Project>
  ...
  <Tasks>
    <Task>
      <UID>1</UID>
      <Name>...</Name>
      <DurationHours>40</DurationHours>
      <PercentComplete>0</PercentComplete>
      <Priority>500</Priority>
      <OutlineLevel>1</OutlineLevel>
      <ParentUID>-1</ParentUID>
      <IsMilestone>0</IsMilestone>
      <IsSummary>0</IsSummary>
      <ConstraintType>ASAP</ConstraintType>
  ...
  <Resources>
    <Resource>
      <UID>1</UID><Name>...</Name><Type>WORK</Type>
      <MaxUnits>1.0</MaxUnits><StandardRate>50</StandardRate>
      <OvertimeRate>0</OvertimeRate><CostPerUse>0</CostPerUse>
    </Resource>
  </Resources>
  <Predecessors>
    <Predecessor>
      ...
      <Type>FS</Type><LagHours>0</LagHours>
    </Predecessor>
  </Predecessors>
  <Assignments>
    <Assignment>
      <TaskUID>1</TaskUID><ResourceUID>1</ResourceUID>
      <Units>1.0</Units><WorkHours>40</WorkHours>
    </Assignment>
  </Assignments>
</Project>
"""
...
def export_project_xml(db: Session, project_id: int) -> bytes:
    ...
    for t in tasks:
        ...
        _txt(te, "UID", t.id)
        _txt(te, "Name", t.name)
        _txt(te, "WBS", t.wbs or "")
        _txt(te, "OutlineLevel", t.outline_level)
        _txt(te, "SortOrder", t.sort_order)
        ...
        _txt(te, "Deadline", t.deadline.isoformat() if t.deadline else "")
        _txt(te, "Priority", t.priority)
        _txt(te, "FixedCost", t.fixed_cost)
        _txt(te, "ActualCost", t.actual_cost)
        _txt(te, "Notes", t.notes or "")
    ...
    for r in resources:
        ...
        _txt(re_, "UID", r.id)
        _txt(re_, "Name", r.name)
        _txt(re_, "Initials", r.initials or "")
        _txt(re_, "Type", r.type.value if r.type else "WORK")
        _txt(re_, "Group", r.group or "")
        _txt(re_, "Email", r.email or "")
        _txt(re_, "MaxUnits", r.max_units)
        _txt(re_, "StandardRate", r.standard_rate)
        _txt(re_, "OvertimeRate", r.overtime_rate)
        _txt(re_, "CostPerUse", r.cost_per_use)
    ...
    for d in deps:
        ...
        _txt(de, "PredecessorUID", d.predecessor_id)
        _txt(de, "SuccessorUID", d.successor_id)
        _txt(de, "Type", d.link_type.value)
        _txt(de, "LagHours", d.lag_hours)
    ...
    for a in assigns:
        ...
        _txt(ae, "TaskUID", a.task_id)
        _txt(ae, "ResourceUID", a.resource_id)
        _txt(ae, "Units", a.units)
        _txt(ae, "WorkHours", a.work_hours)
        _txt(ae, "ActualWorkHours", a.actual_work_hours)
        _txt(ae, "Cost", a.cost)
...
def import_project_xml(db: Session, data: bytes) -> int:
    """Import từ XML — tạo project mới, trả về project_id."""
    root = ET.fromstring(data)
    if root.tag != "Project":
        raise ValueError("XML root phải là <Project>")

    p = Project(
        name=_get(root, "Name", "Imported project"),
    ...
    # Tasks (giữ map UID -> id mới)
    uid_to_id: dict[int, int] = {}
    tasks_xml = list(root.findall("Tasks/Task"))
    # 2 pass: tạo tasks không parent trước, rồi update parent
    for te in tasks_xml:
        uid = int(_get(te, "UID", "0"))
        t = Task(
            project_id=p.id,
            name=_get(te, "Name", "Task"),
            ...
            constraint_type=TaskConstraint(
                _get(te, "ConstraintType", "ASAP") or "ASAP"
            ),
            constraint_date=_parse_dt(_get(te, "ConstraintDate")),
            deadline=_parse_dt(_get(te, "Deadline")),
    ...
    for te in tasks_xml:
        ...
        if parent_uid > 0 and parent_uid in uid_to_id:
            t = db.get(Task, uid_to_id[uid])
            t.parent_id = uid_to_id[parent_uid]
    db.flush()

    # Resources
    res_uid_to_id: dict[int, int] = {}
    for re_ in root.findall("Resources/Resource"):
        uid = int(_get(re_, "UID", "0"))
        r = Resource(
            project_id=p.id,
            name=_get(re_, "Name", "Resource"),
            initials=_get(re_, "Initials") or None,
            type=ResourceType(_get(re_, "Type", "WORK") or "WORK"),
            group=_get(re_, "Group") or None,
            email=_get(re_, "Email") or None,
    ...
    for ae in root.findall("Assignments/Assignment"):
        ...
        if tuid not in uid_to_id or ruid not in res_uid_to_id:
            continue
        db.add(Assignment(
            task_id=uid_to_id[tuid],
            resource_id=res_uid_to_id[ruid],
            units=float(_get(ae, "Units", "1.0") or "1.0"),
            work_hours=float(_get(ae, "WorkHours", "0") or "0"),
            actual_work_hours=float(_get(ae, "ActualWorkHours", "0") or "0"),
