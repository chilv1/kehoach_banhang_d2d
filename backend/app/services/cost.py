...
def recompute_costs(db: Session, project_id: int) -> dict:
    """Tính lại toàn bộ assignment.cost + task.fixed_cost rollup.

    Trả về tổng chi phí project.
    """
    tasks = db.query(Task).filter(Task.project_id == project_id).all()
    by_id = {t.id: t for t in tasks}

    # 1. Cost từng assignment
    assigns = (
        db.query(Assignment)
    ...
    # 2. Total cost của leaf tasks = fixed_cost + sum(assignments)
    ...
    # 3. Rollup cho summary tasks (theo outline_level desc)
    summaries = sorted(
        [t for t in tasks if t.is_summary], key=lambda t: -t.outline_level
    )
    ...
    # 4. Lưu vào actual_cost (planned cost) — note: MS Project có "Cost" và "Actual Cost" riêng
    for t in tasks:
        # dùng field actual_cost làm "computed cost" (rolled), giữ fixed_cost riêng
        # để tránh đè actual khi đang track, chỉ ghi vào nếu chưa có actual
        if t.actual_cost == 0.0:
            t.actual_cost = rolled.get(t.id, 0.0)

    db.commit()
    project_total = sum(
        rolled.get(t.id, 0) for t in tasks if t.parent_id is None
    )
    return {
