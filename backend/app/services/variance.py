...
def task_variance(t: Task, b: BaselineTask | None,
                   cal: CalendarSpec | None = None) -> dict:
    ...
    out = {
        ...
        "start_variance_hours": None,
        "finish_variance_hours": None,
        "duration_variance_hours": None,
        "cost_variance": None,
        "work_variance_hours": None,
        "is_slipping": False,
    }
    if not b:
        return out
    ...
    def _signed(a: datetime | None, base: datetime | None) -> float | None:
    ...
    out["is_slipping"] = bool(
        out["finish_variance_hours"] and out["finish_variance_hours"] > 0
    )
    return out


def project_variance(db: Session, project_id: int,
                      baseline_number: int = 0) -> list[dict]:
    b = (
        db.query(Baseline)
        .filter(Baseline.project_id == project_id,
                Baseline.number == baseline_number)
        .first()
    )
    if not b:
        return []
