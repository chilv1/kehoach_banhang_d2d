...
def _siblings_ordered(db, project_id, parent_id):
    return (db.query(Task).filter(Task.project_id == project_id, Task.parent_id == parent_id)
            .order_by(Task.sort_order, Task.id).all())


def indent(db: Session, task_id: int) -> Task:
    t = db.get(Task, task_id)
    if not t: raise ValueError(f"Task {task_id} không tồn tại")
    sibs = _siblings_ordered(db, t.project_id, t.parent_id)
    idx = next((i for i, x in enumerate(sibs) if x.id == t.id), 0)
    if idx == 0: raise ValueError("Không có sibling phía trước để indent")
...
def outdent(db: Session, task_id: int) -> Task:
    t = db.get(Task, task_id)
    if not t: raise ValueError(f"Task {task_id} không tồn tại")
    if t.parent_id is None: raise ValueError("Đã ở top level")
...
def recompute_summary(db: Session, project_id: int) -> None:
    parents = {t.parent_id for t in db.query(Task).filter(Task.project_id == project_id, Task.parent_id.isnot(None))}
